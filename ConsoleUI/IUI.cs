...
public interface IUI
{
    ...
    /// <summary>
    /// Displays a prompt to the user and allows them to select an option from the provided dictionary.
    /// </summary>
    /// <param name="options">A dictionary where each key represents a valid input character and each value represents the corresponding
    /// option description.</param>
    /// <param name="prompt">An optional message to display before listing the options. If null, no prompt is displayed.</param>
    /// <param name="errorMessage">An optional message to display when the user provides an invalid input. Defaults to "Ogiltigt val, försök igen!"
    /// if not specified.</param>
    /// <returns>The value associated with the selected key in the <paramref name="options"/> dictionary.</returns>
